-- Roll back only the review-gated evidence-candidate write scope.
-- Restore the previous bridge source before running this rollback so the public
-- route fails closed while authorization is narrowed.
...
begin
  ...
  if not found then
    raise exception 'projectos evidence rollback blocked: principal missing';
  end if;
  if not v_principal.is_active
    ...
    raise exception 'projectos evidence rollback blocked: principal identity drift';
  end if;
  if not (
    v_principal.allowed_namespaces <@ array['real_life']::text[]
    and array['real_life']::text[] <@ v_principal.allowed_namespaces
  ) then
    raise exception 'projectos evidence rollback blocked: namespace scope drift';
  end if;
  if not (
    ...
    raise exception 'projectos evidence rollback blocked: unexpected principal scopes';
  end if;
  ...
  if not found
    ...
    raise exception 'projectos evidence rollback blocked: canonical project drift';
  end if;
  ...
  if not found
    ...
    raise exception 'projectos evidence rollback blocked: governed project grant drift';
  end if;
  ...
  if v_other_active_can_propose <> 0 then
    raise exception 'projectos evidence rollback blocked: additional proposal grants exist';
  end if;
end;
$rollback_guard$;
...
begin
  ...
  if v_scopes is null
     or not (
       v_scopes <@ array['memory:health', 'memory:read']::text[]
       and array['memory:health', 'memory:read']::text[] <@ v_scopes
     ) then
    raise exception 'projectos evidence rollback failed: exact scope readback mismatch';
  end if;
end;
$rollback_assertion$;
