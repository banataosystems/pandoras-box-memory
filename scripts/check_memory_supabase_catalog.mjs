...
bad(b.scheduled_jobs?.length===1 && b.scheduled_jobs[0]?.command_persisted===false, "cron evidence incomplete");
...
const raw=JSON.stringify({d,b,c});
...
console.log("Pandora Memory Supabase live catalog evidence verified.");
console.log(`migrations=${m.live_count} relations=${r.relation_count} policies=${p.policy_count} functions=${f.function_count} triggers=${a.application_trigger_count} grants=${acl.grouped_grant_count}`);
